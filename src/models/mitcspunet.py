...
class MiTUnet(nn.Module):
    ...
    def forward(self, x):
        ...
        # for i in range(len(hidden_states_out)):
        #     print(hidden_states_out[i].shape)
        x = self.decoder4(hidden_states_out[4], hidden_states_out[3]) # 16 * feature_size -> 8 * feature_size
        x = self.decoder3(x, hidden_states_out[2]) # 8 * feature_size -> 4 * feature_size
        x = self.decoder2(x, hidden_states_out[1]) # 4 * feature_size -> 2 * feature_size
        x = self.decoder1(x, hidden_states_out[0]) # 2 * feature_size -> feature_size
        x = self.out(x)
